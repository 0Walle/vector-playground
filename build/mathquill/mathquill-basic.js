 ...
 */
(function () {
    var jQuery = window.jQuery, undefined, mqCmdId = 'mathquill-command-id', mqBlockId = 'mathquill-block-id', min = Math.min, max = Math.max;
    function noop() { }
    /**
    ...
    var send = variadic(function (method, args) {
        return variadic(function (obj, moreArgs) {
            if (method in obj)
                return obj[method].apply(obj, args.concat(moreArgs));
        });
    });
    /**
        ...
        _.jQize = function (jQ) {
            ...
            function jQadd(el) {
                if (el.getAttribute) {
                    ...
                    if (cmdId)
                        Node.byId[cmdId].jQadd(el);
                    if (blockId)
                        Node.byId[blockId].jQadd(el);
                }
    ...
    var Cursor = P(Point, function (_) {
        ...
        _.unwrapGramp = function () {
            ...
            gramp.jQ.remove();
            if (gramp[L].siblingDeleted)
                gramp[L].siblingDeleted(cursor.options, R);
            if (gramp[R].siblingDeleted)
                gramp[R].siblingDeleted(cursor.options, L);
        };
    ...
    Controller.open(function (_) {
        ...
        function moveUpDown(self, dir) {
            ...
            if (cursor[R][dirInto])
                cursor.insAtLeftEnd(cursor[R][dirInto]);
            else if (cursor[L][dirInto])
                cursor.insAtRightEnd(cursor[L][dirInto]);
            else {
        ...
        this.onNotify(function (e) { if (e !== 'upDown')
            this.upDownCache = {}; });
        this.onNotify(function (e) { if (e === 'edit')
        ...
        _.deleteDir = function (dir) {
            ...
            if (cursor[L].siblingDeleted)
                cursor[L].siblingDeleted(cursor.options, R);
            if (cursor[R].siblingDeleted)
        ...
        this.onNotify(function (e) { if (e !== 'select')
            this.endSelection(); });
    ...
    Controller.open(function (_) {
        ...
        _.staticMathTextareaEvents = function () {
            var ctrlr = this, root = ctrlr.root, cursor = ctrlr.cursor, textarea = ctrlr.textarea, textareaSpan = ctrlr.textareaSpan;
    ...
    var MathElement = P(Node, function (_, super_) {
        _.finalizeInsert = function (options, cursor) {
            ...
            if (self[R].siblingCreated)
                self[R].siblingCreated(options, L);
            if (self[L].siblingCreated)
                self[L].siblingCreated(options, R);
            self.bubble('reflow');
        };
    });
    /**
            ...
            outer: for (var i = 0, first = l[R] || this.parent.ends[L]; i < str.length; i += 1, first = first[R]) {
                for (var len = min(autoOps._maxLength, str.length - i); len > 0; len -= 1) {
                    ...
                    if (autoOps.hasOwnProperty(word)) {
                        ...
                        if (nonOperatorSymbol(first[L]))
                            first.jQ.addClass('mq-first');
                        if (nonOperatorSymbol(last[R]))
    ...
    var Inequality = P(BinaryOperator, function (_, super_) {
        _.init = function (data, strict) {
            ...
            super_.init.call(this, data['ctrlSeq' + strictness], data['html' + strictness], data['text' + strictness]);
        };
    ...
    forceIERedraw = noop, div = document.createElement('div'), div_style = div.style, transformPropNames = {
        transform: 1,
        WebkitTransform: 1,
        MozTransform: 1,
        OTransform: 1,
    ...
    var SupSub = P(MathCommand, function (_, super_) {
        _.ctrlSeq = '_{...}^{...}';
        _.createLeftOf = function (cursor) {
            if (!cursor[L] && cursor.options.supSubsRequireOperand)
                return;
                ...
                if (this[dir] instanceof SupSub) {
                    ...
                    for (var supsub = 'sub'; supsub; supsub = (supsub === 'sub' ? 'sup' : false)) {
                        ...
                        if (!dest)
                            ...
                            var pt = Point(dest, 0, dest.ends[L]);
        ...
        _.moveTowards = function (dir, cursor, updown) {
            if (cursor.options.autoSubscriptNumerals && !this.sup) {
                ...
                super_.moveTowards.apply(this, arguments);
        };
        _.deleteTowards = function (dir, cursor) {
        ...
        _.deleteSide = function (side, outward, cursor) {
            ...
            if (this.oppBrack(opts, this.ends[L].ends[this.side], side)) { // is ghost,
                ...
                if (sib) { // auto-expand so ghost is at far end
                    ...
                    (outward ? cursor.insDirOf(side, this)
...
//# sourceMappingURL=mathquill-basic.js.map