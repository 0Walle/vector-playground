...
function get_origin(id, env) {
    if (!(id in env))
        return new Vec(0, 0);
    if (env[id].val_origin)
        return env[id].val_origin;
    if (!env[id].origin)
        return new Vec(0, 0);
